...
- (fixed) Fixed issue where sorting or paging active record tables did not preserve form state

## [2.2.12] - 2020-12-30

- (added) Added ability to customize the search input name for active record tables
- (added) Added ability for tables to have unique ID's and only read inputs for their own form submissions
...
- (fixed) Error displayed for parameters sent to add_submenu_page by the AdminPage annotation
- (fixed) Display/layout issue for plugin upload form in WP Admin
