...
class _Framework extends Plugin
{
	...
	{		
		...
		$max_execution_time = max( ( 60 * $max_task_runners ), 60 );
		...
		)
		{
			...
			if ( has_action( $task->action ) )
			{
				...
				try
				{
					...
					{
						/**
						 * Even though we are allowing an overall max_execution_time limit, limit each individual iteration
						 * to a single execution time block.
						 */
						set_time_limit( 60 );
