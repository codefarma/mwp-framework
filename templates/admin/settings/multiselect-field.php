...
<select name='<?php echo $settings->getStorageId() ?>[<?php echo $field->name ?>][]' multiple <?php echo $field->getFieldAttributes() ?>>
	<?php foreach( $selectOptions as $value => $title ) : ?>
		<option value="<?php echo $value ?>" <?php if( in_array( $value, $currentValue ) ){ echo "selected"; } ?>>
			<?php echo $title ?>
		</option>
	<?php endforeach; ?>
</select>
